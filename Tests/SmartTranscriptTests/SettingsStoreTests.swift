...
final class SettingsStoreTests: XCTestCase {
    func testPersistsHotkeyChanges() throws {
        ...
        let updated = HotkeySetting(keyCode: 12, modifiers: UInt32(controlKey | optionKey))
        store.update { $0.copyHotkey = updated }

        let reloaded = SettingsStore(layout: layout)
        XCTAssertEqual(reloaded.settings.copyHotkey, updated)
    }
